...
import { Heart, MessageCircle, Plus, Search, Clock } from "lucide-react";
...
const SAMPLE_POSTS = [
  { id: "1", title: "My Golden Retriever refuses to eat new food — any tips?", content: "We switched to a new brand of kibble and now Bruno just sniffs it and walks away. Been 3 days!", category: "nutrition", created_at: new Date(Date.now() - 3600000).toISOString(), likes_count: 23, comments_count: 8, author: "Priya S." },
  { id: "2", title: "🚨 LOST DOG — Beagle near Jubilee Hills, Hyderabad", content: "Our 2-year-old male Beagle named Biscuit went missing near Road 36. He has a blue collar.", category: "lost-found", created_at: new Date(Date.now() - 7200000).toISOString(), likes_count: 45, comments_count: 12, author: "Rahul M." },
  { id: "3", title: "Best training tips for an 8-week old Labrador puppy?", content: "Just brought home a Lab puppy! First time pet parent here. Looking for basic training tips.", category: "training", created_at: new Date(Date.now() - 86400000).toISOString(), likes_count: 18, comments_count: 15, author: "Anjali K." },
  { id: "4", title: "3 Persian kittens available for adoption in Bengaluru", content: "Found 3 Persian mix kittens, approximately 6 weeks old. Vaccinated and dewormed.", category: "adoption", created_at: new Date(Date.now() - 172800000).toISOString(), likes_count: 67, comments_count: 22, author: "Meera T." },
  { id: "5", title: "What vaccines does my dog need and when?", content: "My vet gave a schedule but I'm confused about which are mandatory vs optional.", category: "health", created_at: new Date(Date.now() - 259200000).toISOString(), likes_count: 34, comments_count: 19, author: "Kiran P." },
];
...
export default function CommunityForum() {
  ...
  const handlePost = () => {
    ...
    const newPost = { id: Date.now().toString(), ...form, created_at: new Date().toISOString(), likes_count: 0, comments_count: 0, author: "You" };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-red-50 p-4 pb-20">
      <div className="max-w-2xl mx-auto">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-12"><MessageCircle size={48} className="mx-auto text-gray-300 mb-3" /><p className="text-gray-400">No posts found</p></div>
        ) : (
                ...
                <Card key={post.id} className="hover:shadow-md transition-shadow cursor-pointer">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <Badge variant="outline" className="text-xs border-pink-200 text-pink-600 bg-pink-50">{catInfo.emoji} {catInfo.label}</Badge>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <button onClick={() => toggleLike(post.id)} className={`flex items-center gap-1 text-sm transition-colors ${liked ? "text-red-500" : "text-gray-400 hover:text-red-400"}`}>
                          <Heart size={16} fill={liked ? "currentColor" : "none"} /><span>{post.likes_count}</span>
                        </button>
                        <div className="flex items-center gap-1 text-gray-400 text-sm"><MessageCircle size={16} /><span>{post.comments_count}</span></div>
                      </div>
